    ...
    for episode in range(episodes):
        ...
        while not done:
            ...
            prediction = model.predict_on_batch(
                observation[np.newaxis, ...])[0].numpy()
            if len(prediction) == 1:
...
if __name__ == "__main__":
    ...
    parser.add_argument("model", default="gym_cartpole_model.h5",
                        nargs="?", type=str, help="Name of model.")
    parser.add_argument("--episodes", default=100,
                        type=int, help="Number of episodes.")
    ...
    total_score = evaluate_model(
        model, args.seed, args.episodes, args.render, report_per_episode=True)

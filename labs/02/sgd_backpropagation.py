    ...
    def train_epoch(self, dataset):
        for batch in dataset.batches(args.batch_size):
            ...
            with tf.GradientTape() as tape:
                ...
                # loss = tf.losses.sparse_categorical_crossentropy(batch["labels"], tf.transpose(probabilities), axis=0)
                # loss = tf.reduce_mean(loss)

                loss = tf.losses.sparse_categorical_crossentropy(
                    batch["labels"], probabilities)
    ...
    def evaluate(self, dataset):
        ...
        for batch in dataset.batches(args.batch_size):
            ...
            correct += tf.math.count_nonzero(
                tf.equal(batch["labels"], tf.argmax(probabilities, axis=1)))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--hidden_layer", default=100,
                        type=int, help="Size of the hidden layer.")
